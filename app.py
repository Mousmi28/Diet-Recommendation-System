...
# Import Langchain with Google Generative AI
...
llm = GoogleGenerativeAI(
    model="gemini-1.5-pro-latest",  # Change model if needed
    google_api_key=api_key,
    model_kwargs={"temperature": 0.7, "max_output_tokens": 100}
)
...
def predict_diet_with_gemini(user_input):
    """Generate diet recommendation using Google Gemini AI."""
...
if st.button("Predict Diet Recommendation"):
    ...
    # Predict diet recommendation using Gemini model
    result = predict_diet_with_gemini(user_input)
